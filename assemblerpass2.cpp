...
int stoii(string v2){
  ...
  else if(v2[0]=='C'){ // multiple byte character
    v2 = v2.substr(2,v2.size()-3);
    int ret=0;
...
bool readIntermediateFile(ifstream& readFile,bool& isComment, int& lineNumber, int& address, int& blockNumber, string& label, string& opcode, string& operand, string& comment){
  ...
  lineNumber = stoii(readTillTab(fileLine,index));
  ...
  if(getFlagFormat(operand)=='#'){
    ...
    /**/&&(CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].exists=='n'))/****/){//Immediate integer value
      ...
      if(if_all_num(tempOperand)){
        immediateValue = stoii(tempOperand);
      }
      else{
...
void assemblerpass2(){
  string tempBuffer;
  intermediateFile.open("intermediatefile.txt");//begin
  if(!intermediateFile){
    cout<<"Unable to open file: intermediatefile.txt"<<endl;
    exit(1);
  }
  ...
  if(!objectFile){
    cout<<"Unable to open file: objectfile.txt"<<endl;
    exit(1);
  }

  ListingFile.open("listingfile.txt");
  if(!ListingFile){
    cout<<"Unable to open file: listingfile.txt"<<endl;
    exit(1);
  }
  writeToFile(ListingFile,"Line\tAddress\tLabel\tOPCODE\tOPERAND\tObjectCode\tComment");

  errorFile.open("errorfile.txt",fstream::app);
  if(!errorFile){
    cout<<"Unable to open file: errorfile.txt"<<endl;
    exit(1);
  }
  ...
  while(opcode!="END"){
    ...
    if(!isComment){
      if(OPTAB[getRealOpcode(opcode)].exists=='y'){
        if(OPTAB[getRealOpcode(opcode)].format==1){
          ...
          if(operand2==operand){//If not two operand i.e. a
            if(getRealOpcode(opcode)=="SVC"){
              objectCode = OPTAB[getRealOpcode(opcode)].opcode + intToStringHex(stoii(operand1),1) + '0';
            }
            ...
            if(REGTAB[operand1].exists=='n'){
              ...
              objectCode = OPTAB[getRealOpcode(opcode)].opcode + REGTAB[operand1].num + intToStringHex(stoii(operand2),1);
            }
        ...
        objectCode = intToStringHex(stoii(operand),6);
      }
      else if(opcode=="BASE"){
...
if(!isComment){
  ...
  writeToFile(objectFile,modificationRecord,false);//Write modification record and readme written by Anonymous
...
int main(){
  cout<<"Enter name of the Input File (with .txt):";
  cin>>fileName;
