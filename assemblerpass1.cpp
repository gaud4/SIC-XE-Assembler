#include "methodheaderfile.cpp" //header files and tables
using namespace std;
...
void evaluateExpression(string expression, bool& relative,string& tempOperand,int lineNumber, ofstream& errorFile,bool& error_flag){
  ...
  for(int i=0;i<expression.length();){
    ...
    if(SYMTAB[singleOperand].exists=='y'){//operand exists or not
      lastOperand = SYMTAB[singleOperand].relative;
      valueTemp = to_string(stringHexToInt(SYMTAB[singleOperand].address));
    }
    else if((singleOperand != "" || singleOperand !="?" ) && if_all_num(singleOperand)){//number is used in expression
      lastOperand = 0;
  ...
  if(!Illegal){
    if(pairCount==1){
      /*relative*/
      relative = 1;
      StringEval tempOBJ(valueString);
...
void assemblerpass1(){
  ...
  writeToFile(errorFile,"       ERRORS IN PASS1       ");  
  ...
  while(opcode!="END"){
    ...
    if(!checkCommentLine(fileLine)){
      ...
      //****************************************************************************************
        ...
        if(opcode=="USE")
        writeData +="- Program Blocks is not implemented";
