...
string expandString(string data,int length,char fillChar,bool back=false){//max size is length , bring it to that size
  if(back){
...
string stringToHexString(const string& input){//ascii to string
...
void readByteOperand(string line,int& index,bool& status,string& data){
  data = "";
  status = true;
  if(line[index]=='C'){//like that string C'TEST STRING' question from slide3
    data += line[index++];
...
StringEval::StringEval(string data){
  storedData = data;
  index=0;
}

int StringEval::getResult(){
  int result = term();
  while(peek()=='+' || peek() == '-'){
    if(get() == '+'){
      result += term();
...
int StringEval::number(){//get the number
  int result = get() - '0';
  while(peek() >= '0' && peek() <= '9'){
